...
/* Realtek private skb fields live after upstream's `tail` clear boundary. */
static __always_inline void rtk_skb_private_clear(struct sk_buff *skb)
{
	memset(&skb->__private_header_start, 0,
...
#if defined(CONFIG_RTK_L34_FLEETCONNTRACK_ENABLE)
	/*
	 * Some stateless/free-shell paths bypass skb_release_head_state().
	 * Match the vendor skb lifetime rules so a private conntrack reference
	 * copied into an skb clone cannot leak on those paths.
	 */
	if (skb->fcIngressData.ct) {
		nf_conntrack_put(&skb->fcIngressData.ct->ct_general);
		skb->fcIngressData.ct = NULL;
	}
#endif
...
static void __copy_skb_header(struct sk_buff *new, const struct sk_buff *old)
{
	...
	/*
	 * Realtek extends struct sk_buff outside the upstream headers group.
	 * These fields must follow an skb clone: the vendor 5.10 tree copies
	 * this exact range.  Omitting it leaves recycled/fclone skb shells with
	 * stale ingress metadata and breaks clone-heavy paths such as TCP TX.
	 */
	memcpy(&new->__private_header_start, &old->__private_header_start,
	       offsetof(struct sk_buff, __private_header_end) -
	       offsetof(struct sk_buff, __private_header_start));
#if defined(CONFIG_RTK_L34_FLEETCONNTRACK_ENABLE)
	if (new->fcIngressData.ct)
		nf_conntrack_get(&new->fcIngressData.ct->ct_general);
#endif
