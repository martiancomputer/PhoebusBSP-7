...
static __inline__ int __nat25_add_pppoe_tag(struct sk_buff *skb, struct pppoe_tag *tag)
{
	...
	memmove((pppoe_hdr_tags(ph) + data_len), pppoe_hdr_tags(ph), ntohs(ph->length));
	ph->length = htons(ntohs(ph->length) + data_len);
	memcpy(pppoe_hdr_tags(ph), tag, data_len);
	return data_len;
}
				...
				if(ph->sid == 0)	// Discovery phase according to tag
				{
					if(ph->code == PADI_CODE || ph->code == PADR_CODE)
					{
						if (priv->pmib->ethBrExtInfo.addPPPoETag) {
							...
							tmp_magic = (char *)&num;
							memcpy(pppoe_tag_data(tag), tmp_magic, 2);
#else
...
#if defined(__ECOS) || defined(__OSK__)
						memcpy(&num, pppoe_tag_data(tag), 2);
						pMagic = &num;
#else
						pMagic = (unsigned short *)pppoe_tag_data(tag);
#endif
