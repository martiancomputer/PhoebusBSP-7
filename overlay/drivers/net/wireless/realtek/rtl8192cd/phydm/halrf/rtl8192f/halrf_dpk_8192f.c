	...
	struct dm_struct *dm)
{
	...
	/*RFE control*/
	/* Phoebus: rfe_type 23 path from the AX10v3 GPL. It drives a different set
	 * of RFE pins during DPK than the generic path below, so without it the
	 * calibration runs against the wrong front-end configuration. */
	if (dm->rfe_type == 23) {
		odm_set_bb_reg(dm, R_0x930, 0x0000FF00, 0x77);
		odm_set_bb_reg(dm, R_0x944, BIT(3) | BIT(2), 0x3);
		odm_set_bb_reg(dm, R_0x940, 0x000000F0, 0x0);
		odm_set_bb_reg(dm, R_0x92c, BIT(3) | BIT(2), 0x1);

		odm_set_bb_reg(dm, R_0x93c, 0x000FF000, 0x77);
		odm_set_bb_reg(dm, R_0x944, BIT(11), 0x1);
		odm_set_bb_reg(dm, R_0x940, BIT(23) | BIT(22), 0x1);
		odm_set_bb_reg(dm, R_0x92c, BIT(27), 0x0);
		odm_set_bb_reg(dm, R_0x944, BIT(4), 0x1);
		odm_set_bb_reg(dm, R_0x940, BIT(9) | BIT(8), 0x1);
		odm_set_bb_reg(dm, R_0x92c, BIT(20), 0x1);
	} else {
	...
	void *dm_void)
{
	...
	/* Phoebus: 23 added (AX10v3 GPL). Without it DPK bails out entirely on this
	 * board's RFE type and TX runs with no pre-distortion calibration. */
	if (!((dm->rfe_type == 3) || (dm->rfe_type == 9) || (dm->rfe_type == 15) || (dm->rfe_type == 19) || (dm->rfe_type == 20) || (dm->rfe_type == 23))) {
		printk("[DPK] Skip DPK due to RFE type != 3 or 9 or 15 or 19 or 20 or 23\n");
		return;
	}
