...
)
{
	if (dm->rfe_type == 7) {
        ...
        /*CUTB+RFE20(PCIE QFN46_NXP_BGS8324_iPA+RXFEM wi RX bypass mode)*/
		odm_set_bb_reg(dm, R_0x103c, 0x70000, 0x7);
		odm_set_bb_reg(dm, R_0x4c, 0x6c00000, 0x0);
		odm_set_bb_reg(dm, R_0x64, BIT(29) | BIT(28), 0x3);
		odm_set_bb_reg(dm, R_0x1038, 0x600000 | BIT(4), 0x0);
		...
		odm_cmn_info_init(dm, ODM_CMNINFO_EXT_LNA, true);
		odm_cmn_info_init(dm, ODM_CMNINFO_EXT_PA, true);
	/* Phoebus: rfe_type 23 added from the AX10v3 GPL (backports-5.2.8-1
	 * rtl8192fe). That is the value the AX10v3 product config selects --
	 * CPTCFG_SLOT_1_RFE_TYPE_23=y -- and this driver, staged from the 8198D
	 * SDK, has every other type in this chain but not 23.
	 *
	 * 0x940/0x944 are the BB's RFE pin-control block: which front-end control
	 * pins are driven and what they output, including the T/R switch. Without
	 * a branch here the function falls through to nothing -- no default, just
	 * a debug print -- so the external front end is never configured. That
	 * matches what the board does: beacons transmit, and the receiver returns
	 * exactly zero (rx_packets, rx_mgnt_pkts, rx_crc_errors, hostapd olbc).
	 *
	 * It also explains why sweeping rfe2g=1 vs 3 produced byte-identical
	 * results: neither value has a branch in this function either. */
	} else if (dm->rfe_type == 17 || dm->rfe_type == 21 ||
		   dm->rfe_type == 22 || dm->rfe_type == 23) {
		/*CUTB+RFE17/21(PCIE QFN46_RTC7646 wi RX bypass mode)*/
		odm_set_bb_reg(dm, R_0x103c, 0x70000, 0x7);
		odm_set_bb_reg(dm, R_0x4c, 0x6c00000, 0x0);
		odm_set_bb_reg(dm, R_0x64, BIT(29) | BIT(28), 0x3);
		odm_set_bb_reg(dm, R_0x1038, 0x600000 | BIT(4), 0x0);
		if (dm->rfe_type == 23) {
			odm_set_bb_reg(dm, R_0x944, MASK12BITS, 0x0000081C);
			odm_set_bb_reg(dm, R_0x940, MASKDWORD,  0x00400100);
		} else {
