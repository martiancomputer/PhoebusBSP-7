...
int rtl8192cd_open(struct net_device *dev)
{
	...
	/* Phoebus debug: wlan1 reports AP-ENABLED but the radio never keys up
	 * (tx_mgnt_pkts:0, rx_mgnt_pkts:0). The WDS branch below returns success
	 * without initialising any hardware when base_addr is 0, and base_addr is
	 * a legacy field modern kernels leave at 0 unless a driver sets it.
	 * Print what we actually get before deciding. Remove once 2.4GHz beacons. */
	printk(KERN_ERR "PHOEBUS-OPEN: %s base_addr=%#lx\n", dev->name, dev->base_addr);
	...
	if (dev->base_addr == 0)
	{
		...
		printk(KERN_ERR "PHOEBUS-OPEN: %s -> WDS early return, NO hw init\n",
		       dev->name);
...
int  rtl8192cd_set_hwaddr(struct net_device *dev, void *addr)
{
	...
	unsigned char phoebus_mac[MACADDRLEN];	/* fallback when the efuse is blank */
#ifdef WDS
	int i;
#endif
#ifdef  CONFIG_WLAN_HAL
	...
	if(!memcmp(zero_mac,p,MACADDRLEN)) {
		...
		/* Not open yet: this is init handing us a blank efuse, not userspace
		 * asking for a bad MAC at runtime. Returning here left GET_MY_HWADDR
		 * and wiphy->perm_addr zero -- and an AP's BSSID *is* its MAC, so the
		 * radio never keyed up: /proc/wlan1/stats showed tx_mgnt_pkts:0 and
		 * rx_mgnt_pkts:0 after 7+ minutes while hostapd happily reported
		 * AP-ENABLED.
		 *
		 * Realtek parts frequently ship with no MAC burnt in the eFuse; the
		 * expected remedy is for the driver to generate one. g6_wifi_driver
		 * already does exactly this ("invalid mac addr ... assign random MAC")
		 * which is why 5GHz works, and upstream rtw89 carries the same fix
		 * ("wifi: rtw89: 8922a: set random mac if efuse contains zeroes").
		 *
		 * NOTE: this is random per boot, so the BSSID changes across reboots
		 * and clients must re-learn the AP. Deriving it from the board's
		 * cmdline ethaddr would be stable; do that once this is confirmed.
		 */
		eth_random_addr(phoebus_mac);
		panic_printk("%s: efuse MAC is blank, assigning random %pM\n",
			     priv->dev->name, phoebus_mac);
		p = phoebus_mac;
	}
...
/* Phoebus: "rfe2g=N" on the kernel cmdline overrides the compiled-in 2.4GHz RF
 * front-end type (see the use site in set_mib_default). __setup() rather than
 * module_param() because this driver is built in, so a module_param would need a
 * KBUILD_MODNAME prefix that is easy to get wrong from U-Boot. -1 = no override. */
static int phoebus_rfe2g_override = -1;

static int __init phoebus_rfe2g_setup(char *str)
{
	if (!str || kstrtoint(str, 0, &phoebus_rfe2g_override))
		phoebus_rfe2g_override = -1;
	return 1;
}
__setup("rfe2g=", phoebus_rfe2g_setup);
...
//Only ONE SLOT, always enable HIGH Power
	...
	/* Phoebus: rfe_type selects the RF front-end configuration -- which AGC
	 * table gets loaded, and how any external LNA/PA is driven. It is fixed at
	 * build time by CONFIG_SLOT_n_RFE_TYPE_m (we have 3), and the driver ships
	 * eight different 8192F AGC tables (bare, extlna, extpa, extlna_type1..3,
	 * sky85201_2L/4L). If it does not match the board, TX can still beacon while
	 * RX goes deaf -- which is exactly what wlan1 does: beacon_ok climbs, but
	 * rx_packets, rx_mgmt_pkts AND rx_crc_errors are all zero, and a live 2.4GHz
	 * channel is never silent to a working receiver.
	 *
	 * Finding the right value meant a rebuild and a 20-minute serial flash per
	 * candidate. Make it a cmdline token instead so the sweep is just reboots:
	 *   rfe2g=6   (-1 or absent = leave the compiled-in value alone)
	 * Always report the value in use so the log says which one was tested. */
	if (phoebus_rfe2g_override >= 0) {
		panic_printk("PHOEBUS-RFE: rfe_type %u -> %d (rfe2g= cmdline override)\n",
			     priv->pmib->dot11RFEntry.rfe_type, phoebus_rfe2g_override);
		priv->pmib->dot11RFEntry.rfe_type = phoebus_rfe2g_override;
	} else {
		panic_printk("PHOEBUS-RFE: rfe_type=%u (compiled-in; override with rfe2g=N)\n",
			     priv->pmib->dot11RFEntry.rfe_type);
	}
