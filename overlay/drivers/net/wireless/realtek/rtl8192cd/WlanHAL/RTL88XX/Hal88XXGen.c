...
)
{
	...
	if ( IS_HARDWARE_TYPE_8192F(Adapter) ) {
		if (IS_HAL_TEST_CHIP(Adapter)) {
			...
			*((unsigned long*)(val)) = (unsigned long)data_PHY_REG_PG_8192F_start;
			...
			/* Phoebus: front-end specific table (AX10v3 GPL). Without this the
			 * generic table was used whatever CONFIG_SLOT_0_RFE_TYPE_n said. */
			printk("[PHY_REG_PG_8192F_type3]\n");
	...
	case HW_VAR_POWERLIMITFILE_START: {
		...
		if ( IS_HARDWARE_TYPE_8192F(Adapter) ) {
			if (HAL_RFE_TYPE == 3) {
				printk("[TXPWR_LMT_92FE_type3]\n");
				*((unsigned long*)(val)) = (unsigned long)data_TXPWR_LMT_92FE_type3_start;
			} else {
