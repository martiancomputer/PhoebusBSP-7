...
/* Linux 7.3 removed strncpy() from the kernel API. This vendor tree still
 * relies on its zero-padding semantics, so provide a driver-local shim. */
#ifndef strncpy
static inline char *rtw_legacy_strncpy(char *dest, const char *src, size_t n)
{
	size_t i;
...
#define strncpy(dest, src, n) rtw_legacy_strncpy((dest), (src), (n))
#endif
...
struct atm_stainfo {
	/* MAC_ADDR_LEN, not MACADDRLEN: the latter is only defined by
	 * rtw_br_ext.h, so this unrelated airtime struct failed to build once
	 * CONFIG_BR_EXT was turned off. Both are 6. */
	u8	hwaddr[MAC_ADDR_LEN];
	u32	atm_statime;
};
