...
/* strncpy() left the kernel API in Linux 7.3. The imported G6 driver uses
 * its historical padding semantics throughout, so keep the compatibility
 * local to this vendor driver rather than restoring it kernel-wide. */
#ifndef strncpy
#define strncpy(dest, src, n) _os_strncpy((dest), (src), (n))
#endif
#if 1
