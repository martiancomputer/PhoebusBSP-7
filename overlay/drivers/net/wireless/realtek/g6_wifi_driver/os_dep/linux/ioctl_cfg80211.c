	...
	u64 *cookie)
{
	...
	/* A NULL chan is not an error: cfg80211 passes it to mean "transmit on
	 * the current operating channel" (see the comment in nl80211_tx_mgmt(),
	 * and mac80211's ieee80211_mgmt_tx() which falls back the same way).
	 * hostapd depends on this -- it sends probe/auth/assoc responses with
	 * freq=0, so NL80211_ATTR_WIPHY_FREQ is absent and chandef.chan is NULL.
	 * Rejecting it here made every probe response fail with -EINVAL
	 * ("handle_probe_req: send failed"), so stations saw a beacon but could
	 * never finish a scan or associate.
	 */
	if (chan == NULL) {
		tx_ch = pmlmeext->cur_channel;
		if (tx_ch == 0) {
			ret = -EINVAL;
			goto exit;
		}
		tx_freq = rtw_ch2freq(tx_ch);
	} else {
		tx_ch = (u8)ieee80211_frequency_to_channel(chan->center_freq);
		tx_freq = chan->center_freq;
	}
 ...
 */
/* 7.1 additionally moved the key and station ops from net_device * to
 * wireless_dev *. Note set_default_key did NOT change and still takes a
 * net_device * -- do not "fix" it to match its neighbours. */
static int ph_add_key(struct wiphy *w, struct wireless_dev *wdev, int link_id,
	u8 key_index, bool pairwise, const u8 *mac_addr, struct key_params *params)
{ return cfg80211_rtw_add_key(w, wdev->netdev, key_index, pairwise, mac_addr, params); }

static int ph_get_key(struct wiphy *w, struct wireless_dev *wdev, int link_id,
	u8 keyid, bool pairwise, const u8 *mac_addr, void *cookie,
	void (*cb)(void *, struct key_params *))
{ return cfg80211_rtw_get_key(w, wdev->netdev, keyid, pairwise, mac_addr, cookie, cb); }

static int ph_del_key(struct wiphy *w, struct wireless_dev *wdev, int link_id,
	u8 key_index, bool pairwise, const u8 *mac_addr)
{ return cfg80211_rtw_del_key(w, wdev->netdev, key_index, pairwise, mac_addr); }
...
static int ph_set_default_mgmt_key(struct wiphy *w, struct wireless_dev *wdev,
	int link_id, u8 key_index)
{ return cfg80211_rtw_set_default_mgmt_key(w, wdev->netdev, key_index); }

/* station ops: net_device * -> wireless_dev * in 7.1 */
static int ph_get_station(struct wiphy *w, struct wireless_dev *wdev,
	const u8 *mac, struct station_info *sinfo)
{ return cfg80211_rtw_get_station(w, wdev->netdev, mac, sinfo); }

static int ph_add_station(struct wiphy *w, struct wireless_dev *wdev,
	const u8 *mac, struct station_parameters *params)
{ return cfg80211_rtw_add_station(w, wdev->netdev, mac, params); }

static int ph_del_station(struct wiphy *w, struct wireless_dev *wdev,
	struct station_del_parameters *params)
{ return cfg80211_rtw_del_station(w, wdev->netdev, params); }

static int ph_change_station(struct wiphy *w, struct wireless_dev *wdev,
	const u8 *mac, struct station_parameters *params)
{ return cfg80211_rtw_change_station(w, wdev->netdev, mac, params); }

static int ph_dump_station(struct wiphy *w, struct wireless_dev *wdev,
	int idx, u8 *mac, struct station_info *sinfo)
{ return cfg80211_rtw_dump_station(w, wdev->netdev, idx, mac, sinfo); }
...
struct cfg80211_ops rtw_cfg80211_ops = {
	...
	.add_station = ph_add_station,
	.del_station = ph_del_station,
	.change_station = ph_change_station,
	.dump_station = ph_dump_station,
