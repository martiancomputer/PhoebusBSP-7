...
/* Must be 2 ("auto"), not 1, for hostapd-driven AP mode. At 1, rtw_check_beacon_data()
 * takes neither branch of its VHT arm: the first is gated on MLME_IS_MESH() ("allow only
 * mesh temporarily before VHT IE checking is ready") and the second on REGSTY_IS_11AC_AUTO,
 * so vht_option stays _FALSE and rtw_vht_ies_detach() then STRIPS VHT Capability/Operation
 * out of the beacon -- and update_sta_vht_info_apmode() clears the per-STA vht_option, so
 * the assoc response loses VHT too.
 *
 * hostapd answers probe requests itself (cfg80211_rtw_mgmt_tx), so probe responses keep
 * their VHT IEs. The client therefore learns "VHT AP" from the probe response, associates,
 * gets an assoc response and beacons with HT only, and mac80211 kills the link:
 *   AP bug: VHT capa missing from AssocResp
 *   AP <bssid> appears to change mode (expected VHT, found HT) in beacon, disconnect
 *
 * At 2 the driver detaches hostapd's VHT IEs and attaches its own, built from actual PHY
 * capability -- which is also more correct than what hostapd emits here (its VHT cap info
 * was all zeroes on the wire).
 */
