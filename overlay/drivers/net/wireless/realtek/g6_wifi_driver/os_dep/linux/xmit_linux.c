...
int rtw_xmit_entry(struct sk_buff *pkt, _nic_hdl pnetdev)
{
	...
	/* Phoebus: on 6.18 an association produces two PHOEBUS-EAPOL-TX lines (1/4
	 * and 3/4) and the 4-way completes in ~35ms. On 7.1.5 that printk never
	 * fires and hostapd times the handshake out after ~4.2s. rtw_xmit_entry,
	 * _rtw_xmit_entry and the whole of rtw_xmit.c are byte-identical between
	 * the two trees, so the frame is lost either before this entry point or
	 * between here and update_attrib_sec_info -- which need opposite fixes.
	 *
	 * This fires the instant the netdev is handed the frame, before any driver
	 * state is consulted, so it separates the two cleanly: silence means the
	 * kernel never delivered it (look at packet_snd/qdisc), a line here with no
	 * matching PHOEBUS-EAPOL-TX means the driver swallowed it in between.
	 *
	 * Also reports netif_running, because the early return below hands a bare
	 * -ENETDOWN back to dev_hard_start_xmit, which expects a NETDEV_TX_* code. */
	if (pkt && pkt->len >= ETH_HLEN &&
	    *(__be16 *)(pkt->data + 2 * ETH_ALEN) == __constant_htons(ETH_P_EAPOL))
		printk(KERN_ERR "PHOEBUS-XMIT-ENTRY: %s EAPOL len=%u running=%d\n",
		       pnetdev->name, pkt->len, netif_running(pnetdev) ? 1 : 0);
...
//#endif
	...
	if (drop_pkt == 1) {
		if (MLME_IS_AP(padapter) && is_dhcp_pkt(pkt->data)) {
			/* MAC_ADDR_LEN: MACADDRLEN only exists via rtw_br_ext.h,
			 * which is gone now that CONFIG_BR_EXT=n. Both are 6. */
