...
struct action_handler OnAction_tbl[] = {
	...
	{RTW_WLAN_CATEGORY_ROBUST_AV,        "ACTION_ROBUST_AV",        &OnAction_robust_av},
#endif
	{RTW_WLAN_CATEGORY_VENDOR_SPECIFIC_PROTECTED, "ACTION_VENDOR_SPECIFIC_PROTECTED", &on_action_vsp},
};
...
unsigned int OnProbeReq(_adapter *padapter, union recv_frame *precv_frame)
{
	...
	core_map_send_client_event(padapter, padapter->mac_addr, TUNNELED_MSG_PROBEREQ, precv_frame->u.hdr.attrib.phy_info.signal_strength, len, pframe);
#endif

#if defined(CONFIG_RTW_MULTI_AP)
	core_map_update_unassoc_sta_metric(padapter, precv_frame);
#endif

#ifdef RECORD_ENV_STAINFO
	rtw_record_env_stainfo(padapter, precv_frame);
#endif
	...
	if(padapter->registrypriv.wifi_mib.fastcon_en && rtw_fastcon_rx_check(padapter, precv_frame)== SUCCESS){
		DBG_COUNTER(padapter->rx_logs.core_rx_fastcon);
...
#ifdef CONFIG_IOCTL_CFG80211
	if (MLME_IS_AP(padapter) &&
		((GET_CFG80211_REPORT_MGMT(adapter_wdev_data(padapter), IEEE80211_STYPE_PROBE_REQ) == _TRUE) || wps_ie)) {
...
_continue:
	...
	wmode = rtw_get_wmode_by_ie(pframe + WLAN_HDR_A3_LEN + _PROBEREQ_IE_OFFSET_, len - WLAN_HDR_A3_LEN - _PROBEREQ_IE_OFFSET_);
	...
	if (check_fwstate(pmlmepriv, WIFI_ASOC_STATE) == _TRUE &&
		...
		issue_probersp(padapter, get_sa(pframe), _FALSE, wmode);
...
_issue_probersp:
		if (((check_fwstate(pmlmepriv, WIFI_ASOC_STATE) == _TRUE &&
			...
			issue_probersp(padapter, get_sa(pframe), is_valid_p2p_probereq, wmode);
		}
...
void rtw_get_pkt_tsf(u8 *pframe, u64 *tsf)
{
	u8 *pIE;
	u32 *pbuf;
	u64 pkt_val;
	...
	pkt_val = le32_to_cpu(*(pbuf + 1));

	pkt_val = pkt_val << 32;

	pkt_val |= le32_to_cpu(*pbuf);
...
unsigned int OnProbeRsp(_adapter *padapter, union recv_frame *precv_frame)
{
...
#if defined(CONFIG_RTW_MULTI_AP) && defined(CONFIG_NSB_MULTI_AP_EVENT)
	core_map_send_client_event(padapter, padapter->mac_addr, TUNNELED_MSG_PROBERES, precv_frame->u.hdr.attrib.phy_info.signal_strength, precv_frame->u.hdr.len, pframe);
#endif
	...
	if( MLME_IS_AP(padapter) )
	{
		...
		if (pkt_len < IEEE80211_3ADDR_LEN + ie_offset) {
			RTW_INFO("handle_assoc(prob_rsp) - too short payload (len=%lu)"
		...
		if (elems.he_operation && (elems.he_operation_len <= HE_OPER_ELE_MAX_LEN)) {
			...
			u8 ap_bs;
			u8 bss_disabled = 0;
			u8 factor = 1;  //rtw_random32() % 2;
			// u8 ap_bc_en = adapter->registrypriv.wifi_mib.bs_detect;	
#if 0
			struct rtw_phl_port_tsf tsf_val;
			u64 cur_tsf, pkt_tsf;

			rtw_get_pkt_tsf(pframe, &pkt_tsf);

			rtl_phl_cmd_get_cur_tsf(phl, &tsf_val,
				padapter->phl_role,
				PHL_CMD_DIRECTLY, 0);

			cur_tsf = tsf_val.tsf_h;
			cur_tsf = cur_tsf << 32;
			cur_tsf |= tsf_val.tsf_l;

			if (pkt_tsf > cur_tsf) {
#else
			if (factor) {
#endif
				u8 *bss_info = elems.he_operation + 4;

				ap_bs = GET_HE_OP_BSS_COLOR_INFO_BSS_COLOR(bss_info);
				bss_disabled - GET_HE_OP_BSS_COLOR_INFO_BSS_COLOR_DISABLE(bss_info);

				if (!bss_disabled && ap_bs > 0) {
					padapter->bs_map |= (1ULL << ap_bs);
				}
			}
		}
	}
...
static void rtw_wifi_diag_onBeacon_bss(_adapter *adapter, union recv_frame *precv_frame, WLAN_BSSID_EX *pbss)
{
...
unsigned int OnBeacon(_adapter *padapter, union recv_frame *precv_frame)
{
	...
	_rtw_memset(&pbss_parse, 0, sizeof(WLAN_BSSID_EX));
	if (collect_bss_info(padapter, precv_frame, &pbss_parse) == _SUCCESS) {
#ifdef CONFIG_WIFI_DIAGNOSIS
		rtw_wifi_diag_onBeacon_bss(padapter, precv_frame, &pbss_parse);
#endif
#ifdef AP_NEIGHBOR_INFO
		rtw_add_ap_neighbor_info(padapter, precv_frame, &pbss_parse);
#endif
#ifdef CTC_WIFI_DIAG
		ctcwifi_diag_log(padapter, pbss_parse.MacAddress, 0, "Beacon", (pframe+WLAN_HDR_A3_LEN), (len-WLAN_HDR_A3_LEN));
#endif
	}
...
unsigned int OnAuth(_adapter *padapter, union recv_frame *precv_frame)
{
	...
	if(pmlmeext->bstart_bss == _FALSE){
		DBGP(" bstart_bss is FALSE, onAuth return\n");
		return _FAIL;
	}
	...
	RTW_PRINT("+OnAuth sa="MAC_FMT", rssi = %d\n",  MAC_ARG(get_addr2_ptr(pframe)), precv_frame->u.hdr.attrib.phy_info.signal_strength);
	...
	if (prxattrib->phy_info.signal_strength != 0)
		pstat->phl_sta->hal_sta->rssi_stat.assoc_rssi = prxattrib->phy_info.signal_strength;
...
void rtw_parse_vendor_vw_sta(_adapter *padapter, struct sta_info *pstat)
{
	...
	if (pstat->vendor == HT_IOT_PEER_VERIWAVE &&
		(padapter->vw_enable == VW_MODE_NONE ||
			padapter->vw_enable == VW_MODE_TP ||
			padapter->vw_enable == VW_MODE_LATENCY)) {
		if (padapter->vw_enable == VW_MODE_NONE)
			padapter->vw_enable = VW_MODE_TP;

		if (padapter->dvobj->tx_mode == 0 || padapter->dvobj->tx_mode == 2) {
			...
			if (padapter->swq_one_sta && _rtw_memcmp(padapter->swq_one_sta_mac, "\x0\x0\x0\x0\x0\x0", ETH_ALEN) == _TRUE)
				_rtw_memcpy(padapter->swq_one_sta_mac, pstat->phl_sta->mac_addr, ETH_ALEN);
			#endif
			...
			/* If antdiv is enabled, fix A1 if testing TC/VW */
			if (GET_PRIMARY_ADAPTER(padapter)->registrypriv.wifi_mib.antdiv)
				rtw_phl_antdiv_pause(dvobj->phl, 1, 1);
#endif
		}
	}
...
#ifdef CONFIG_IOCTL_CFG80211
static inline u8 issue_assoc_rsp_by_driver(struct sta_info *psta)
{
	return (!psta->process_assoc_req_by_hapd);
}

static inline u8 send_assoc_rsp_by_hostapd(struct sta_info *psta)
{
	u8 ret = (!!psta->process_assoc_req_by_hapd);

	/* Reset to default here since sta_info might be reused later */
...
unsigned int OnAssocReq(_adapter *padapter, union recv_frame *precv_frame)
{
	...
	if(pmlmeext->bstart_bss == _FALSE){
		DBGP(" bstart_bss is FALSE, onAssoc return\n");
		return _FAIL;
	}
	...
	/* Phoebus: a failing connect shows three "IEEE 802.11: associated" lines
	 * and no AP-STA-CONNECTED; a succeeding one shows exactly one. Log the
	 * sequence number and retry bit so we can tell WHICH of the two it is:
	 *   differing seq, retry=0  -> the client is making fresh attempts, i.e.
	 *                              our assoc RESPONSE never reached it
	 *   same seq, retry=1       -> the client retransmitted and our dedup
	 *                              (which needs BOTH conditions) let it past
	 * Those need opposite fixes, so stop guessing and measure. */
	printk(KERN_ERR "PHOEBUS-ASSOCREQ: "MAC_FMT" seq=%d retry=%d\n",
	       MAC_ARG(get_addr2_ptr(pframe)),
	       precv_frame->u.hdr.attrib.seq_num, GetRetry(pframe) ? 1 : 0);
#ifdef CONFIG_RTW_MBO
	/* AP is under BSS termination duration*/
	if (pmlmepriv->mbopriv.bss_termination_phase == BSS_TERMINATION_DUR) {
	...
	if ((pstat->bpairwise_key_installed == _TRUE && (pstat->flags & WLAN_STA_MFP))
		...
		RTW_INFO("STA " MAC_FMT " Authentication failed [temporary reject]\n",
...
#ifdef CONFIG_IOCTL_CFG80211
		{
			u8 *passoc_req =  rtw_zmalloc(pkt_len);
			u32 assoc_req_len = 0;

			if (passoc_req) {
				assoc_req_len = pkt_len;
				_rtw_memcpy(passoc_req, pframe, pkt_len);
				rtw_cfg80211_indicate_sta_assoc(padapter, passoc_req, assoc_req_len);
				rtw_mfree(passoc_req, assoc_req_len);
			}
		}
#else
		issue_asocrsp(padapter, status, pstat, (frame_type == WIFI_ASSOCREQ) ? WIFI_ASSOCRSP : WIFI_REASSOCRSP);
	...
	/* Check total sta num limitation */
	if (pdvobj->total_sta_num_limit > 0 && get_dvobj_total_sta_num(pdvobj) >= pdvobj->total_sta_num_limit) {
		RTW_INFO("Reject " MAC_FMT " by STA limitation:%u (Exceed the upper limit of supported clients)\n",
			MAC_ARG(pstat->phl_sta->mac_addr), pdvobj->total_sta_num_limit);
		status = WLAN_STATUS_AP_UNABLE_TO_HANDLE_NEW_STA;
#ifdef CTC_WIFI_DIAG
		ctcwifi_assoc_err(padapter, pstat->phl_sta->mac_addr, "Rejected by STA limitation [Exceed the upper limit of supported clients...]\n");
#endif
		goto OnAssocReqFail;
	}
	...
	if (primary_adapter->multi_ap_mode & MAP_MODE_FRONT_AP) {
		if(rtw_blacklist_find_in_list(&padapter->black_list, get_addr2_ptr(pframe))) {
			status = _STATS_FAILURE_;
			RTW_INFO("STA " MAC_FMT " Rejected by blacklist [auth abort by MULTI_AP]\n", MAC_ARG(pstat->phl_sta->mac_addr));
#ifdef CTC_WIFI_DIAG
...
#ifdef CONFIG_RTW_MULTI_AP
	if (padapter->multiap_max_device_reached) {
		status = _STATS_FAILURE_;
		RTW_INFO("STA " MAC_FMT " Association ignored by multi ap [max device reached]\n", MAC_ARG(pstat->phl_sta->mac_addr));
#ifdef CTC_WIFI_DIAG
		ctcwifi_assoc_err(padapter, pstat->phl_sta->mac_addr, "Association ignored by multi ap [max device reached]\n");
#endif
		goto OnAssocReqFail;
	}
#endif
	...
	if (elems.ssid == NULL
		...
		status = _STATS_FAILURE_;
		RTW_INFO("STA " MAC_FMT " Capability checking failed [SSID does not match]\n", MAC_ARG(pstat->phl_sta->mac_addr));
#ifdef CTC_WIFI_DIAG
	...
	if (status != _STATS_SUCCESSFUL_) {
		RTW_INFO("STA " MAC_FMT " Capability checking failed [Supported rates is wrong]\n", MAC_ARG(pstat->phl_sta->mac_addr));
#ifdef CTC_WIFI_DIAG
	...
	if (status != _STATS_SUCCESSFUL_) {
		RTW_INFO("STA " MAC_FMT " Capability checking failed [Security IE is wrong]\n", MAC_ARG(pstat->phl_sta->mac_addr));
#ifdef CTC_WIFI_DIAG
	...
	status = rtw_ap_parse_sta_dh_para_ie(padapter, pstat, &elems);
	if (status != _STATS_SUCCESSFUL_) {
		RTW_INFO("STA " MAC_FMT " Capability checking failed [OWE related IEs are wrong]\n", MAC_ARG(pstat->phl_sta->mac_addr));
#ifdef CTC_WIFI_DIAG
		ctcwifi_assoc_err(padapter, pstat->phl_sta->mac_addr, "Capability checking failed [OWE related IEs are wrong]\n");
#endif
		goto OnAssocReqFail;
	}
	...
	if (_band_steering_block_chk(padapter, get_sa(pframe))) {
		status = _STATS_REFUSED_TEMPORARILY_;
		RTW_INFO("["MAC_FMT"]Association ignored by band steering [block list check]", MAC_ARG(pstat->phl_sta->mac_addr));
#ifdef CTC_WIFI_DIAG
	...
	if (elems.mdie && elems.mdie_len) {
		...
		if (rtw_ft_enable(padapter)) {
			pstat->process_assoc_req_by_hapd |= ASSOC_REQ_HAS_MOBILITY_DOMAIN;
		}
	}
...
#endif
#ifdef RTW_STA_BWC
		padapter->need_decide_tp_lmt = _TRUE;
#endif /* RTW_STA_BWC */
	}
	_rtw_spinunlock_bh(&pstapriv->asoc_list_lock);
	...
	if (pstat && (pstat->state & WIFI_FW_ASSOC_SUCCESS) && (_STATS_SUCCESSFUL_ == status)) {
		...
		if (primary_adapter->multi_ap_mode & MAP_MODE_FRONT_AP) {
			...
			if (pstat->assoc_req_length <= sizeof(pstat->assoc_req_body)) {
...
#endif
#ifdef CONFIG_RTL_WIFI_OFFLOAD
		if (RTL_WFO_IS_OFLD(pdvobj)) {
			rtl_wfo_check_res(padapter);
			#ifdef CONFIG_VW_REFINE
			apply_pe_amsdu_para(padapter);
		...
		#ifdef CONFIG_IOCTL_CFG80211
		if (!issue_assoc_rsp_by_driver(pstat)) {
			RTW_PRINT("recv %s-req (" MAC_FMT "), offload to hostapd\n",
		...
		core_map_send_client_event(padapter, padapter->mac_addr, TUNNELED_MSG_ASSOREQ, precv_frame->u.hdr.attrib.phy_info.signal_strength, pkt_len, pframe);
#else
		if(frame_type==WIFI_ASSOCREQ)
...
unsigned int OnAssocRsp(_adapter *padapter, union recv_frame *precv_frame)
{
...
#if defined(CONFIG_RTW_MULTI_AP) && defined(CONFIG_NSB_MULTI_AP_EVENT)
	core_map_send_client_event(padapter, padapter->mac_addr, TUNNELED_MSG_ASSORES, precv_frame->u.hdr.attrib.phy_info.signal_strength, pkt_len, pframe);
#endif
	...
	if (status > 0) {
		...
		if (status == WLAN_STATUS_INVALID_PMKID) {
			rtw_del_cached_pmkid(padapter, get_ta(pframe));
		}
...
unsigned int on_action_wnm(_adapter *adapter, union recv_frame *rframe)
{
	...
	case RTW_WLAN_ACTION_WNM_BTM_QUERY:
...
#ifdef CONFIG_RTW_MULTI_AP_R2
#ifdef CONFIG_NSB_MULTI_AP_EVENT
			core_map_send_client_event(adapter, adapter->mac_addr, TUNNELED_MSG_BTMQUERY, rframe->u.hdr.attrib.phy_info.signal_strength, frame_len, frame);
#else
		...
		break;
#ifdef CONFIG_RTW_AP_BS_DECT
	case RTW_WLAN_ACTION_WNM_EVT_RPT:
		RTW_PRINT("WNM: RTW_WLAN_ACTION_WNM_EVT_RPT recv. (status=%d)\n",
				frame_body[3]);
		if (MLME_IS_AP(adapter)) {
			rtw_wnm_process_evt_rtp(adapter, sta, frame_body, frame_body_len);
		}
		ret = _SUCCESS;
		break;
#endif
...
#if defined(CONFIG_RTW_MULTI_AP) && defined(CONFIG_NSB_MULTI_AP_EVENT)
			core_map_send_client_event(adapter, adapter->mac_addr, TUNNELED_MSG_BTMRSP, rframe->u.hdr.attrib.phy_info.signal_strength, frame_len, frame);
#endif
		}
		/* fall through */
	default:
...
/**
 * rtw_rx_ampdu_size_by_sta - Get the target RX AMPDU buffer size for the specific @adapter
 * @adapter: the adapter to get target RX AMPDU buffer size
 * @psta: the sta to make the IOT decision
 * @addba_req_buf_size: return value should not greater than addba_req_buf_size
 *
 * Returns: the target RX AMPDU buffer size
 */
u16 rtw_rx_ampdu_size_by_sta(_adapter *adapter, struct sta_info *psta, u16 addba_req_buf_size)
{
	...
	/* don't exceed request from sta */
	if (size > addba_req_buf_size) {
		size = addba_req_buf_size;
	}
		...
		case RTW_WLAN_ACTION_ADDBA_RESP: /* ADDBA response */
			...
			if (status == 0) {
...
#ifdef CONFIG_RTL_WIFI_OFFLOAD
				u8 prv_agg_en_bitmap, prv_tx_amsdu_enable;

				prv_agg_en_bitmap = psta->htpriv.agg_enable_bitmap;
				prv_tx_amsdu_enable = psta->htpriv.tx_amsdu_enable;
#endif /* CONFIG_RTL_WIFI_OFFLOAD */
				RTW_INFO("ADDBA_RESP: agg_enable %u for TID=%d\n", buf_num, tid);
				...
				if (RTL_WFO_IS_OFLD(padapter->dvobj) &&
					...
					(psta->htpriv.agg_enable_bitmap != prv_agg_en_bitmap ||
					psta->htpriv.tx_amsdu_enable != prv_tx_amsdu_enable) &&
					rtw_core_add_ofld_txsc(padapter, psta, psta->phl_sta->mac_addr) == SUCCESS) {
					/* Turn on TX ofld once txsc added */
...
#ifdef CONFIG_RTW_AP_BS_DECT
u8 rtw_pick_new_color(u64 bitmap)
{
	u8 i, j, max, start;
	u64 test_bitmap = 0;

	max = 64;
	start = (rtw_random32() & 0x3F);
...
void rtw_check_and_switch_color(_adapter *adapter, u64 bitmap)
{
	...
	u8 new_color = 0;
	u64 old_color_bitmap = 0;
	u8 he_op_ie_ex = WLAN_EID_EXTENSION_HE_OPERATION;
	_adapter *padapter = NULL;
	u8 i;

	old_color_bitmap = 1ULL << proto_cap->bsscolor;

	if (old_color_bitmap & bitmap)
		new_color = rtw_pick_new_color(bitmap);

	if (new_color > 0 && new_color < 64) {
		rfctl->bcc_color = new_color;
		rfctl->bcc_cntdown = 5;
		rfctl->bcc_set_ie = 1;
		...
		for (i = 0; i < dvobj->iface_nums; i++) {
			...
			if (!padapter ||
				padapter->netif_up == _FALSE || !MLME_IS_AP(padapter))
			continue;
			...
			proto_cap->bsscolor = new_color;

			update_beacon(padapter, WLAN_EID_EXTENSION,
					&he_op_ie_ex, _TRUE, 0);
                }
	}
...
void rtw_wnm_process_evt_rtp(_adapter *padapter, struct sta_info *psta, u8 *pframe, u32 frame_len)
{
	u8 dialog_token;
	u8 status_code, color;
	struct wnm_evt_rpt_elem *ent_elm = NULL;
	u64 bitmap;
	dialog_token = pframe[2];

	ent_elm = (struct wnm_evt_rpt_elem *) (pframe+3);

	if (ent_elm->eid != RTW_WNM_IE_EVT_RPT ||
		ent_elm->status != 0)
		return;

	switch(ent_elm->type) {
	case RTW_WNM_EVT_COLOR_COLLISION:
		bitmap = RTW_GET_LE64(ent_elm->u.bss_color_collision.color_bitmap);
		rtw_check_and_switch_color(padapter, bitmap);
		break;
	case RTW_WNM_EVT_COLOR_IN_USE:
		color = ent_elm->u.bss_color_in_use.color;
		if (color > 63) {
			RTW_PRINT("invalid color = %d \n", color);
			return;
		}
		padapter->bs_map |= 1ULL  << color;
		break;
	defaut:
		RTW_PRINT("WNM event report type =%d not found\n", ent_elm->type);
	}
}
#endif
...
#ifdef CONFIG_RTW_MULTI_AP
#ifdef CONFIG_RTL_WIFI_OFFLOAD
	psta->is_btm_resp_recved = _TRUE;
	if (RTL_WFO_IS_OFLD(padapter->dvobj)) {
		/* Redirect pkts back to driver */
		psta->rps_pe_id = RTL_WFO_ARM_ID;
		rtl_wfo_ipc_cfg_sta_info(padapter, psta,
			(PE_ROLE_DOE | PE_ROLE_AOE), RTL_WFO_OP_UPDATE);
	}
#endif
	if (frame_len - frlen) {
...
void rtw_wnm_issue_action(_adapter *padapter, u8 action, u8 reason)
{
	...
	u8 category=0, dialog_token =0, termination_delay=0, *pframe = NULL;
	u16 *fctrl = NULL;
...
void rtw_wnm_check_frames_tx(_adapter *padapter, const u8 **dump_buf, size_t *dump_len)
{
	...
	struct mlme_priv *mlme = &padapter->mlmepriv;
	struct mlme_ext_info *mlmeinfo = &padapter->mlmeextpriv.mlmext_info;
	struct roam_nb_info *nb_info = &mlme->nb_info;
	void *phl = padapter->dvobj->phl;
	u8 *new_buf = NULL;
	u8 *tmp_buf = NULL;
	...
	u16 disassoc_imminent_timeout = 0;
	u32 disassoc_imminent_ms = 0;
	...
	if(subelement_ptr[sizeof(struct rtw_ieee80211_hdr_3addr) + 3] & DISASSOC_IMMINENT) {
		disassoc_imminent = 1;
		disassoc_imminent_timeout = le16_to_cpu(*((u16 *)&subelement_ptr[sizeof(struct rtw_ieee80211_hdr_3addr) + 4]));
	}
	...
	if (disassoc_imminent) {
		psta->mbopriv.disassoc_imminent_step = DISASSOC_IMMINENT_STEP_NONE;
		disassoc_imminent_ms = disassoc_imminent_timeout * mlmeinfo->bcn_interval;
		if (DISASSOC_IMMINENT_TIMEOUT_WKARD < disassoc_imminent_ms)
			disassoc_imminent_ms -= DISASSOC_IMMINENT_TIMEOUT_WKARD;
		_set_timer(&psta->mbopriv.disassoc_imminent_timer, disassoc_imminent_ms);
	}
...
{
	unsigned char                category, action, dialog_token, desc_len, element_id;
	u8                          *pframe = precv_frame->u.hdr.rx_data;
	u32			    packet_len = precv_frame->u.hdr.len;
	sint			    len;
	struct sta_info             *psta = precv_frame->u.hdr.psta;
	//_adapter_link               *adapter_link = precv_frame->u.hdr.adapter_link;
	//struct rtw_wifi_role_link_t *rlink = adapter_link->rlink;
	u8			    scs_idx;
	struct mlme_ext_priv        *dev_mlmeext = &(padapter->mlmeextpriv);
	struct mlme_ext_info        *dev_mlmeinfo = &(dev_mlmeext->mlmext_info);
	//struct link_mlme_ext_priv   *pmlmeext = &(adapter_link->mlmeextpriv);
	//struct mlme_ext_info        *pmlmeinfo = &(pmlmeext->mlmext_info);
	u16			    scsreq_status, mscsreq_status;
	struct rtw_ieee80211_hdr_3addr *whdr = (struct rtw_ieee80211_hdr_3addr *)pframe;
	u8 *frame_body = pframe + sizeof(struct rtw_ieee80211_hdr_3addr);
	...
	if (!MLME_IN_AP_STATE(dev_mlmeinfo))
		if (!(dev_mlmeinfo->state & WIFI_FW_ASSOC_SUCCESS))
			return _SUCCESS;
	...
	RTW_ERR("\033[1;35m [%s] =====category==== %d \n \033[m", __FUNCTION__, category);

	if (category == RTW_WLAN_CATEGORY_ROBUST_AV) {

		action = frame_body[1];

		switch (action) {
			case RTW_WLAN_ACTION_SCS_REQUEST:
				dialog_token = frame_body[2];
				desc_len = frame_body[4];
				RTW_ERR("\033[1;35m [%s] =====category==== %d packet_len:%d, len:%d, scs_len:%d\n \033[m",
					__FUNCTION__, category, packet_len, len, desc_len);
				scsreq_status = process_scs_req(padapter, (u8 *)&(frame_body[3]), len - 3, psta);
				rtw_addscsrsp_cmd(padapter, psta, dialog_token);

				for (scs_idx = 0; scs_idx < MAX_SCS_DESCRIPTOR; scs_idx++) {
					if ((psta->phl_sta->scs_info[scs_idx].scsid != 0) && (psta->phl_sta->scs_info[scs_idx].req_type != SCS_PROCESSED)) {
						if (psta->phl_sta->scs_info[scs_idx].req_type == SCS_REMOVE)
							psta->phl_sta->scs_info[scs_idx].scsid = 0;
						else
							psta->phl_sta->scs_info[scs_idx].req_type = SCS_PROCESSED;
					}
				}
#ifdef CONFIG_RTW_MULTI_AP_R4
				core_map_send_ras_scs_req_notify(padapter, padapter->mac_addr, psta->phl_sta->mac_addr, frame_body, len);
#endif
				break;

			case RTW_WLAN_ACTION_MSCS_REQUEST:
				dialog_token = frame_body[2];
				element_id = frame_body[3];
				desc_len = frame_body[4];
				RTW_ERR("\033[1;35m [%s] =====category==== %d packet_len:%d, len:%d, mscs_len:%d\n \033[m",
					__FUNCTION__, category, packet_len, len, desc_len);
				mscsreq_status = process_mscs_req(padapter, (u8 *)&(frame_body[3]), len - 3, psta);
				rtw_addmscsrsp_cmd(padapter, (struct addMSCSRsp_parm){
					.psta = psta,
					.dialog_token = dialog_token,
					.status = mscsreq_status,
				});

#ifdef CONFIG_RTW_MULTI_AP_R4
				core_map_send_ras_mscs_req_notify(padapter, padapter->mac_addr, psta->phl_sta->mac_addr, frame_body, len);
#endif
				break;
...
unsigned int on_action_vsp(_adapter *padapter, union recv_frame *precv_frame)
{
	...
	category = frame_body[0];
	if (category != RTW_WLAN_CATEGORY_VENDOR_SPECIFIC_PROTECTED)
		goto exit;

	ret = rtw_core_vsp_qos_on_action(padapter, frame_body, frame_body_len, psta);
...
static void _mgnt_txsts(struct rtw_txfb_t *txfb)
{
	...
	if (txfb->ctx) {
		sctx = (struct submit_ctx *)txfb->ctx;
		_rtw_spinlock_bh(&txfb->lock);
		rtw_sctx_done_err(&sctx,
				  (  txfb->txsts == TX_STATUS_TX_DONE
				   ? RTW_SCTX_DONE_SUCCESS
				   : RTW_SCTX_DONE_UNKNOWN));
		_rtw_spinunlock_bh(&txfb->lock);
	} else {
		RTW_ERR("%s: NULL sctx\n", __FUNCTION__);
	}
}
...
#ifdef CONFIG_RTW_AP_BS_DECT
	bcc_anoce_eid_ext = WLAN_EID_EXTENSION_BCC_ANOCE;

	if ((pie_get = rtw_get_ie_ex(pie_start, total_ielen,
		WLAN_EID_EXTENSION, &bcc_anoce_eid_ext, 1, NULL, &get_ielen)) != NULL)
	{
		bcn_cmn->ofst_bs_chg = (u32)(pie_get - pframe) + 3;

		RTW_INFO("BCC beacon, pktlen: %u, pframe: %p, pie_start: %p, "
			"pie_get: %p, total_ielen: %u, csa_offset: %u, value: %u\n",
			pattrib->pktlen, pframe, pie_start, pie_get, total_ielen,
			bcn_cmn->ofst_bs_chg, (u8)*(pframe + bcn_cmn->ofst_bs_chg));
	} else {
		bcn_cmn->ofst_bs_chg = 0;
	}
#endif

	if (bcn_cmn->bcn_added) {
#if 0
		/* bcn_cmn->bcn_interval has been updated before, no need to be updated again */
		if (mib->bcnint) {
		...
		} else if (bcn_cmn->bcn_interval != pmlmeinfo->bcn_interval) {
		...
		if (padapter->dvobj->dynamic_bcnint)
			bcn_cmn->bcn_interval = padapter->dvobj->dynamic_bcnint;
		else
			bcn_cmn->bcn_interval = pmlmeinfo->bcn_interval;
...
void issue_beacon(_adapter *padapter, int timeout_ms)
{
	...
	struct wifidirect_info	*pwdinfo = &(padapter->wdinfo);
#endif /* CONFIG_P2P */
#if defined(CONFIG_CSA_IE) || defined(CONFIG_RTW_AP_BS_DECT)
...
#ifdef CONFIG_RTW_AP_BS_DECT
		if (rfctl->bcc_set_ie) {
			pframe = rtw_set_ie_ex_bc_change(pframe,
				&(pattrib->pktlen), rfctl->bcc_color, rfctl->bcc_cntdown);
		}
#endif
		goto _issue_bcn;
	}
...
void _rebuild_probersp_ext_cap_ie(_adapter *padapter, struct xmit_frame	*pmgntframe, u8 *pframe, u8 wmode)
{
	struct mlme_ext_priv *pmlmeext = &padapter->mlmeextpriv;
	struct pkt_attrib *pattrib = &pmgntframe->attrib;
	struct mlme_ext_info *pmlmeinfo = &(pmlmeext->mlmext_info);
	...
	p = rtw_get_ie(pie + _FIXED_IE_LENGTH_, _EXT_CAP_IE_, &ielen, ttl_ielen - _FIXED_IE_LENGTH_);
	if (!p || ielen <= _WKARD_EXT_CAP_LENGTH_)
		return;

	rm_ret = rtw_ies_remove_ie(pie, &ttl_ielen, _BEACON_IE_OFFSET_, _EXT_CAP_IE_, NULL, 0);
	if (rm_ret == _FAIL)
		return;

	pframe -= (ielen +2 );
	pattrib->pktlen -= (ielen +2);

	p = rtw_get_ie(cur_network->IEs + _BEACON_IE_OFFSET_, _EXT_CAP_IE_, &ielen, (cur_network->IELength - _BEACON_IE_OFFSET_));
	if (!p || ielen <= _WKARD_EXT_CAP_LENGTH_)
		return;

	ielen = _WKARD_EXT_CAP_LENGTH_;
	_rtw_memcpy(pframe, p, ielen + 2);
	*(pframe + 1) = ielen;
	pframe += (ielen + 2);
	pattrib->pktlen += (ielen + 2);
}
...
void issue_probersp(_adapter *padapter, unsigned char *da, u8 is_valid_p2p_probereq, u8 wmode)
{
	...
	*(fctrl) = 0;
	...
	if (MLME_IN_AP_STATE(pmlmeinfo)) {
		...
		/* re-build probe-rsp for iot issue */
		_rebuild_probersp_ext_cap_ie(padapter, pmgntframe, pframe, wmode);
	} else
#endif
	{
...
void issue_asocrsp(_adapter *padapter, unsigned short status, struct sta_info *pstat, int pkt_type)
{
	...
	/* Phoebus: issue_asocrsp already logs itself a few lines below
	 * ("issue_asocrsp sta=... status=..."), so the only case not covered is
	 * this silent early return -- which would mean we never send a response
	 * at all and the client's retries are entirely justified. */
	if (rtw_rfctl_is_tx_blocked_by_ch_waiting(adapter_to_rfctl(padapter))) {
		printk(KERN_ERR "PHOEBUS-ASSOCRSP: BLOCKED by ch_waiting, not sent\n");
		return;
	}

	RTW_INFO("%s sta="MAC_FMT" status=%d\n", __FUNCTION__, MAC_ARG(pstat->phl_sta->mac_addr), status);
	...
	*(fctrl) = 0;
	...
	if (pmlmepriv->ext_capab_ie_len > 0) {
		...
		if (pbuf && ie_len > 0) {
			/* re-build assoc-rsp for iot issue */
			if (is_supported_24g(padapter->registrypriv.band_type) &&
				!is_supported_he(pstat->phl_sta->wmode))
				ie_len = (ie_len > _WKARD_EXT_CAP_LENGTH_) ? _WKARD_EXT_CAP_LENGTH_ : ie_len;
...
#ifdef CONFIG_QOS_MANAGEMENT
	if (GET_EXT_CAPABILITY_ELE_QOS_MAP(pstat->ext_capab_ie_data))
...
static void _null_data_txsts(struct rtw_txfb_t *txfb)
{
	...
	if ((psta->flags & WLAN_STA_A4) && psta->multiap_profile) {
		RTW_INFO("DEBUG[%s]: A4 sta=" MAC_FMT ": psta->keep_alive_trycnt=%d, psta->state=0x%x\n", __func__, MAC_ARG(psta->phl_sta->mac_addr), psta->keep_alive_trycnt, psta->state);
		return;
	}
#endif /* CONFIG_RTW_A4_STA */
...
/* when wait_ack is ture, this function shoule be called at process context */
static int _issue_om_with_nulldata(_adapter *padapter, struct sta_info *ptarget_sta, u16 tid, int wait_ack)
{
	int ret = _FAIL;
	struct xmit_frame			*pmgntframe;
	struct pkt_attrib			*pattrib;
	unsigned char				*pframe;
	unsigned char 				*da = ptarget_sta->phl_sta->mac_addr;
	struct rtw_ieee80211_hdr	*pwlanhdr;
	u16							*fctrl, *qc;
	u32 						*htc;
	struct xmit_priv			*pxmitpriv = &(padapter->xmitpriv);
	...
	*(fctrl) = 0;
	...
	htc = (u32 *)(pframe + pattrib->hdrlen - 2 + sizeof(unsigned short));

	SetVHTSupport(htc, ptarget_sta->vhtpriv.vht_option);

	SetHESupport(htc, ptarget_sta->hepriv.he_option);
	
	SetControlID(htc, OM_ID);
	
	SetRxNSS(htc, (ptarget_sta->phl_sta->asoc_cap.nss_tx - 1));
	
	SetChBw(htc, ptarget_sta->phl_sta->chandef.bw);
	
	SetUlMuDisable(htc, 1);
	
	SetTxNSTS(htc, (ptarget_sta->phl_sta->asoc_cap.nss_rx - 1));

	SetSeqNum(pwlanhdr, pmlmeext->mgnt_seq);
	pmlmeext->mgnt_seq++;
	set_frame_sub_type(pframe, WIFI_QOS_DATA_NULL);

	pframe += sizeof(struct rtw_ieee80211_hdr_3addr_qos_htc) + a4_shift;
	pattrib->pktlen = sizeof(struct rtw_ieee80211_hdr_3addr_qos_htc) + a4_shift;
...
int issue_om_with_nulldata(_adapter *padapter, struct sta_info *ptarget_sta, u16 tid, int try_cnt, int wait_ms)
{
	...
	do {
		ret = _issue_om_with_nulldata(padapter, ptarget_sta, tid, wait_ms > 0 ? _TRUE : _FALSE);
		...
		case RTW_WLAN_ACTION_ADDBA_REQ:
			...
			if (   psta->hepriv.he_option
			    && (padapter->phl_role->proto_role_cap.num_ampdu > 64)) {
			    	if (psta->vendor == HT_IOT_PEER_FIBOCOM_PDA)
			    		BA_para_set = (64 << 6); /* 64 buffer size */
			    	else
					BA_para_set = (padapter->phl_role->proto_role_cap.num_ampdu << 6); /* buffer size from cap */
			} else
				BA_para_set = (64 << 6); /* 64 buffer size */
			...
			if (padapter->registrypriv.ctc_dscp) {
				...
				if (RTL_WFO_IS_OFLD(adapter_to_dvobj(padapter)))
					start_seq = (padapter->dvobj->wfo_dev->doe_sw_seq[psta->phl_sta->macid * 8 + tid] + 1) & 0xfff;
				else
#endif /* CONFIG_RTL_WIFI_OFFLOAD */
					start_seq = (psta->sta_xmitpriv.txseq_tid[tid & 0x07] +1) & 0xfff;
			} else
#endif
...
#ifdef CONFIG_QOS_MANAGEMENT
static int issue_action_robust_av(_adapter *padapter,
				  unsigned char *raddr,
...
{
	int                         ret = _FAIL;
	u8                          category = RTW_WLAN_CATEGORY_ROBUST_AV;
	u16                         start_seq;
	u16                         cputole16_status;
	struct xmit_frame           *pmgntframe;
	struct pkt_attrib           *pattrib;
	u8                          *pframe;
	struct rtw_ieee80211_hdr    *pwlanhdr;
	...
	u8			    scs_idx;
	u8			    scs_info_len;
	u8                          scs_info[MAX_SCS_INFO_BYTES] = {0};

	if (rtw_rfctl_is_tx_blocked_by_ch_waiting(adapter_to_rfctl(padapter))) // TODO   link always 0?
		goto exit;
	...
	*(fctrl) = 0;
	...
	psta = rtw_get_stainfo(pstapriv, raddr);
	if (psta == NULL) {
		RTW_ERR("NULL STA to issue action robust av\n");
		rtw_free_xmitframe(&padapter->xmitpriv, pmgntframe);
		return _FAIL;
	}
	if (category == RTW_WLAN_CATEGORY_ROBUST_AV) {
		switch (action) {
			...
			case RTW_WLAN_ACTION_SCS_RESPONSE: {
				struct addSCSRsp_parm *pp = (struct addSCSRsp_parm *)pparam;
				pframe = rtw_set_fixed_ie(pframe, 1, &(pp->dialog_token), &(pattrib->pktlen));
				pframe = rtw_set_fixed_ie(pframe, 1, &(pp->scs_count), &(pattrib->pktlen));
				/*SCS Status List*/
				for (scs_idx = 0; scs_idx < MAX_TEMP_SCS_STATUS; scs_idx++) {
					if (pp->scs_status[scs_idx].scsid == 0) continue;
					pframe = rtw_set_fixed_ie(pframe, 1, &(pp->scs_status[scs_idx].scsid), &(pattrib->pktlen));
					cputole16_status = cpu_to_le16(pp->scs_status[scs_idx].status);
					pframe = rtw_set_fixed_ie(pframe, 2, (unsigned char *)(&cputole16_status), &(pattrib->pktlen));
					RTW_INFO("\033[1;35m [%s] ===== scsid : %d, status : %d ====  \033[m\n", __FUNCTION__,
						pp->scs_status[scs_idx].scsid , pp->scs_status[scs_idx].status);
				}
				break;
			}
#if 0
			/* SCS Descriptor List for BE */
			if (pmlmeinfo->EHT_enable) {
				for (scs_idx = 0; scs_idx < MAX_TEMP_SCS_STATUS; scs_idx++) {
					if ((paddscsstatus[scs_idx].status == SCS_REQ_SUCCESS) && (paddscsstatus[scs_idx].req_type == SCS_ADD)) {
						scs_info_len = rtw_phl_build_scs_descriptor(psta->phl_sta, paddscsstatus[scs_idx].scsid, SCS_ADD, scs_info);
						if (scs_info_len > 0) {
							pframe = rtw_set_fixed_ie(pframe, scs_info_len, (unsigned char *)(&scs_info), &(pattrib->pktlen));
						}
					}
				}
			}
			...
			case RTW_WLAN_ACTION_MSCS_RESPONSE: {
				struct addMSCSRsp_parm *pp = (struct addMSCSRsp_parm *)pparam;
				pframe = rtw_set_fixed_ie(pframe, 1, &(pp->dialog_token), &(pattrib->pktlen));
				cputole16_status = cpu_to_le16(pp->status);
				pframe = rtw_set_fixed_ie(pframe, 2, (unsigned char *)&cputole16_status, &(pattrib->pktlen));
				break;
			}
			default:
...
inline void issue_addscs_rsp(_adapter *adapter, unsigned char *ra,
			     struct addSCSRsp_parm *pparam)
{
	issue_action_robust_av(adapter, ra, RTW_WLAN_ACTION_SCS_RESPONSE, (void *)pparam, _FALSE);

	RTW_INFO(FUNC_ADPT_FMT" ra="MAC_FMT" dialog_token=%u\n",
		FUNC_ADPT_ARG(adapter), MAC_ARG(ra), pparam->dialog_token);
}

inline void issue_addmscs_rsp(_adapter *adapter, unsigned char *ra,
			     struct addMSCSRsp_parm *pparam)
{
	issue_action_robust_av(adapter, ra, RTW_WLAN_ACTION_MSCS_RESPONSE, (void *)pparam, _FALSE);
...
#ifdef CONFIG_RTL_WIFI_OFFLOAD
			else if (RTL_WFO_IS_OFLD(adapter->dvobj))
				rtl_wfo_ipc_update_rxofld_info(adapter, sta, 0, tid, 0);
#endif /* CONFIG_RTL_WIFI_OFFLOAD */
		}
	} else if (initiator == 1) {
		/* originator */
...
u8 collect_bss_info(_adapter *padapter, union recv_frame *precv_frame, WLAN_BSSID_EX *bssid)
{
	...
	if (*(p + 1)) {
		...
		#ifdef CONFIG_WLAN_EVENT_INDICATE_GENL
		if ((!padapter->last_name_conflict_time ||
				(padapter->last_name_conflict_time && rtw_get_passing_time_ms(padapter->last_name_conflict_time) >= 60000)) &&
			!strcmp(cur_network->network.Ssid.Ssid, bssid->Ssid.Ssid)) {
			padapter->last_name_conflict_time = rtw_get_current_time();
			genl_wlan_IndicateEvent(padapter, EVENT_NAME_CONFLICT, &cur_network->network.Ssid);
		}
		#endif /* CONFIG_WLAN_EVENT_INDICATE_GENL */
	} else
	...
	#ifdef CONFIG_WLAN_EVENT_INDICATE_GENL
	if ((!padapter->last_bssid_conflict_time ||
				(padapter->last_bssid_conflict_time && rtw_get_passing_time_ms(padapter->last_bssid_conflict_time) >= 60000)) &&
		_rtw_memcmp(bssid->MacAddress, cur_network->network.MacAddress, ETH_ALEN) == _TRUE) {
			padapter->last_bssid_conflict_time = rtw_get_current_time();
			genl_wlan_IndicateEvent(padapter, EVENT_BSSID_CONFLICT, bssid);
		}
	#endif /* CONFIG_WLAN_EVENT_INDICATE_GENL */
...
void rtw_set_hw_after_join(struct _ADAPTER *a, int join_res)
{
	...
	if (join_res < 0) {
		...
		if (RTL_WFO_IS_OFLD(a->dvobj)) {
			/* Redirect pkts back to driver */
			if (a->dvobj->wfo_en & RTL_WFO_TX_OFLD) {
				rtl_wfo_del_staid_to_fc(sta);
				rtl_wfo_clear_fc_staid(a, sta);
			}
			rtl_wfo_ipc_cfg_sta_info(a, sta,
					(PE_ROLE_PPE | PE_ROLE_DOE | PE_ROLE_AOE),
					RTL_WFO_OP_DELETE);
...
#ifdef CONFIG_RTL_WIFI_OFFLOAD
	if (RTL_WFO_IS_OFLD(a->dvobj)) {
		rtl_wfo_pe_id_decision(a, sta);
		/* update VXD's BSSID to PE */
		rtl_wfo_ipc_cfg_adapter_info(a, RTL_WFO_OP_UPDATE);
		rtl_wfo_ipc_cfg_sta_info(a, sta,
				(PE_ROLE_PPE | PE_ROLE_DOE | PE_ROLE_AOE),
				RTL_WFO_OP_ADD);
	}
...
void check_sa_query_mgmt_tx(_adapter *padapter, const u8 *buf, u32 len)
{
	...
	RTW_INFO("tx sa_query[%d] to sta [%pm]\n", psta->sa_query_cnt, psta->phl_sta->mac_addr);

	if (psta->sa_query_cnt == SA_QUERY_MAX_NUM) {
...
void sa_query_timer_hdl(void *ctx)
{
	...
	if (check_fwstate(pmlmepriv, WIFI_AP_STATE) == _TRUE) {
		RTW_INFO("no one rsp sa_query, clear sta [%pm]\n", psta->phl_sta->mac_addr);
...
u8 stop_ap_hdl(_adapter *adapter)
{
	...
	adapter->hapd_scanned_time = 0;

#ifdef CONFIG_RTW_80211K
	if (adapter->rmpriv.enable) {
		struct rm_priv *prmpriv = &adapter->rmpriv;
...
#ifdef CONFIG_RTL_WIFI_OFFLOAD
		if (RTL_WFO_IS_OFLD(padapter->dvobj))
			rtl_wfo_ipc_update_rxofld_info(padapter, psta,
					pparm->start_seq, pparm->tid, pparm->size);
#endif /* CONFIG_RTL_WIFI_OFFLOAD */
	}
...
#ifdef CONFIG_RTL_WIFI_OFFLOAD
	if (RTL_WFO_IS_OFLD(a->dvobj))
		rtl_wfo_ipc_update_rxofld_info(a, sta, 0, parm->tid, 0);
#endif /* CONFIG_RTL_WIFI_OFFLOAD */
...
{
	struct addSCSRsp_parm *pparm = (struct addSCSRsp_parm *)pbuf;
	struct sta_priv *pstapriv = &padapter->stapriv;
	struct sta_info *psta;
	u8 ret = _TRUE;
	...
	issue_addscs_rsp(padapter, pparm->addr, pparm);
exit:
	return H2C_SUCCESS;
}
...
{
	struct addMSCSRsp_parm *pparm = (struct addMSCSRsp_parm *)pbuf;
	struct sta_priv *pstapriv = &padapter->stapriv;
	struct sta_info *psta = pparm->psta;
	u8 ret = _TRUE;

	if (!psta)
		goto exit;

	issue_addmscs_rsp(padapter, psta->phl_sta->mac_addr, pparm);
exit:
	return H2C_SUCCESS;
}
#endif
...
#else /* !(RTW_WKARD_AP_CMD_DISPATCH) */
u8 set_tx_beacon_cmd(_adapter *padapter, u8 flags)
{
	struct cmd_priv	*pcmdpriv = &(adapter_to_dvobj(padapter)->cmdpriv);
	struct mlme_ext_priv	*pmlmeext = &padapter->mlmeextpriv;
	...
	struct Tx_Beacon_param *ptxBeacon_parm = NULL;
...
_free_txbeacon_parm:
	rtw_mfree((void *)ptxBeacon_parm, sizeof(struct Tx_Beacon_param));

_exit:
	return res;
}
#endif /* RTW_WKARD_AP_CMD_DISPATCH */
...
#define DUMP_ADAPTERS_STATUS 0
	...
	if (join_res >= 0 && ret <= 0) {
		join_res = -1;
		dump_adapters_status(RTW_DBGDUMP , dvobj);
		RTW_WARN("%s Can't find union about ch, bw, ch_offset of all linked/linking interfaces; join_res = %d, ret =%d\n"
			, __func__, join_res, ret);
		//rtw_warn_on(1);
	}
...
int rtw_chk_start_clnt_join(_adapter *adapter, u8 *ch, u8 *bw, u8 *offset)
{
...
#ifdef CONFIG_CONCURRENT_MODE
	{
		...
		if (rtw_mi_get_ch_setting_union_no_self(adapter, &u_ch, &u_bw, &u_offset) <= 0) {
			dump_adapters_status(RTW_DBGDUMP , dvobj);
			RTW_INFO(FUNC_ADPT_FMT" no others union except self\n", FUNC_ADPT_ARG(adapter));
		} else {
			RTW_INFO(FUNC_ADPT_FMT" others union except self:%u,%u,%u\n", FUNC_ADPT_ARG(adapter), u_ch, u_bw, u_offset);
		}
...
#ifdef CONFIG_RTW_AP_BS_DECT
u8 set_bc_hdl(_adapter *adapter, unsigned char *pbuf)
{
	struct rtw_wifi_role_t *wrole = adapter->phl_role;
	struct protocol_cap_t *proto_cap = &(wrole->proto_role_cap);
	...
	struct dvobj_priv *dvobj = adapter->dvobj;
	_adapter *padapter = NULL;
	u8 he_op_ext = WLAN_EID_EXTENSION_HE_OPERATION;
	u8 i;

	if (rfctl->bcc_color) {
		rfctl->bcc_set_ie = 0;

		rtw_phl_cmd_wrole_change(phl, wrole,
			WR_CHG_BSS_COLOR,
			&rfctl->bcc_color, 1,
			PHL_CMD_DIRECTLY, 0);
		...
		for (i = 0; i < dvobj->iface_nums; i++) {
			...
			proto_cap = &(wrole->proto_role_cap);
 			...
 			proto_cap->bc_disabled = 0;
			proto_cap->bsscolor = rfctl->bcc_color;

			update_beacon(padapter,
				WLAN_EID_EXTENSION, &he_op_ext, _TRUE, 0);
		}
	}
...
int rtw_sae_preprocess(_adapter *adapter, const u8 *buf, u32 len, u8 tx)
{
	...
	if (tx && MLME_IS_AP(adapter)) {
		psta = rtw_get_stainfo(pstapriv, GetAddr1Ptr(buf));
		if (psta) {
			psta->sae_status = status;
		}
	}
...
void rtw_assoc_preprocess(_adapter *padapter, const u8 *buf, u32 len, int type)
{
	...
	if (!send_assoc_rsp_by_hostapd(pstat))
		goto exit;
	...
	if (ptr_tmp && ptr_tmp[2] == WLAN_EID_EXTENSION_OWE_DH_PARAM) {
...
#ifdef RECORD_ENV_STAINFO
struct env_sta_info env_stainfo_table[ENV_STAINFO_NUM];
DEFINE_SPINLOCK(env_stainfo_lock);

void _fill_env_stainfo_entry(_adapter *padapter, union recv_frame *precv_frame, struct env_sta_info *entry)
{
	struct rtw_ieee802_11_elems elems;
	struct sta_info *psta = precv_frame->u.hdr.psta;
	enum wlan_mode max_mode_support;
	enum channel_width max_bw_support = CHANNEL_WIDTH_20;
	u8 *pframe = precv_frame->u.hdr.rx_data;
	u8 *pos = pframe + WLAN_HDR_A3_LEN;
	int left = precv_frame->u.hdr.len - WLAN_HDR_A3_LEN;

	rtw_ieee802_11_parse_elems(pos, left, &elems, 1);

	if (elems.he_capabilities_len)
		max_mode_support = WLAN_MD_11AX;
	else if (elems.vht_capabilities_len)
		max_mode_support = WLAN_MD_11AC;
	else if (elems.ht_capabilities_len)
		max_mode_support = WLAN_MD_11N;
	else if (WIFI_ROLE_IS_ON_5G(padapter))
		max_mode_support = WLAN_MD_11A;
	else if (elems.supp_rates_len)
		max_mode_support = WLAN_MD_11G;
	else
		max_mode_support = WLAN_MD_11B;

	if (elems.ht_capabilities_len) {
		if (GET_HT_CAP_ELE_CHL_WIDTH(elems.ht_capabilities))
			max_bw_support = CHANNEL_WIDTH_40;
	}

	if (WIFI_ROLE_IS_ON_5G(padapter)) {
		if (elems.vht_capabilities_len) {
			switch (GET_VHT_CAPABILITY_ELE_CHL_WIDTH(elems.vht_capabilities)) {
			case 1: /* 160MHz */
				max_bw_support = CHANNEL_WIDTH_160;
				break;
			case 2: /* 80+80 */
				max_bw_support = CHANNEL_WIDTH_80_80;
				break;
			}
		}

		if (elems.vht_operation_len && max_bw_support < CHANNEL_WIDTH_80) {
			if (GET_VHT_OPERATION_ELE_CHL_WIDTH(elems.vht_operation))
				max_bw_support = CHANNEL_WIDTH_80;
		}
	}

	if (elems.he_capabilities_len) {
		u8 *he_phy_cap = elems.he_capabilities + 1 + HE_CAP_ELE_MAC_CAP_LEN;

		if (WIFI_ROLE_IS_ON_5G(padapter)) {
			if (GET_HE_PHY_CAP_SUPPORT_CHAN_WIDTH_SET(he_phy_cap) & BIT(2))
				max_bw_support = CHANNEL_WIDTH_160;
			else if (GET_HE_PHY_CAP_SUPPORT_CHAN_WIDTH_SET(he_phy_cap) & BIT(1))
				max_bw_support = CHANNEL_WIDTH_80;
		} else {
			if (GET_HE_PHY_CAP_SUPPORT_CHAN_WIDTH_SET(he_phy_cap) & BIT(0))
				max_bw_support = CHANNEL_WIDTH_40;
		}
	}

	if (WIFI_ROLE_IS_ON_24G(padapter))
		entry->support_band |= BAND_CAP_2G;
	if (WIFI_ROLE_IS_ON_5G(padapter))
		entry->support_band |= BAND_CAP_5G;
	if (WIFI_ROLE_IS_ON_6G(padapter))
		entry->support_band |= BAND_CAP_6G;

	if (entry->used == _FALSE ||
		max_bw_support > entry->max_bw_support)
		entry->max_bw_support = max_bw_support;
	if (entry->used == _FALSE ||
		max_mode_support > entry->max_mode_support)
		entry->max_mode_support = max_mode_support;
	entry->conneted = psta ? _TRUE : _FALSE;
...
{
	struct dvobj_priv *dvobj = adapter_to_dvobj(padapter);
	struct env_sta_info *entry = &env_stainfo_table[0];
	u8 *pframe = precv_frame->u.hdr.rx_data;
	u8 *da = GetAddr1Ptr(pframe);
	u8 *sa = get_addr2_ptr(pframe);
	u8 i;
	u16 unused_index = ENV_STAINFO_NUM;
	u16 replace_index = ENV_STAINFO_NUM;
	boolean found = _FALSE;

	if (!padapter->registrypriv.wifi_mib.record_env_stainfo ||
		!MacAddr_isBcst(da))
		return;

	_rtw_spinlock_bh(&env_stainfo_lock);

	/* find target entry in env_stainfo_table */
	for (i = 0; i < ENV_STAINFO_NUM; i++) {
		if (entry[i].used == _FALSE) {
			/* record first unused index */
			if (unused_index == ENV_STAINFO_NUM)
				unused_index = i;
			continue;
		}

		/* record longest not updated and no connect entry */
		if (!entry[i].conneted) {
			if (replace_index == ENV_STAINFO_NUM)
				replace_index = i;
			else if (entry[replace_index].last_update > entry[i].last_update)
				replace_index = i;
		}

		if (_rtw_memcmp(entry[i].mac, sa, ETH_ALEN) == _TRUE) {
			found = _TRUE;
			break;
		}
	}

	/* replace entry in extra_sta_info_table */
	if (found == _FALSE) {
		if (unused_index != ENV_STAINFO_NUM)
			i = unused_index;
		else if (replace_index != ENV_STAINFO_NUM)
			i = replace_index;
		else
			goto exit;

		entry[i].used = _FALSE;
		entry->support_band = 0;
		_rtw_memcpy(entry[i].mac, sa, ETH_ALEN);
	}

	_fill_env_stainfo_entry(padapter, precv_frame, &entry[i]);

exit:
	_rtw_spinunlock_bh(&env_stainfo_lock);
}
...
struct env_sta_info *lookup_env_stainfo_table(_adapter *padapter, u8 *mac)
{
	...
	for (i = 0; i < ENV_STAINFO_NUM; i++) {
		...
		if (_rtw_memcmp(entry[i].mac, mac, ETH_ALEN) == _TRUE)
			return &entry[i];
	}
...
int proc_dump_env_stainfo_table(struct seq_file *m, void *v)
{
	...
	RTW_PRINT_SEL(m, "id mac max_mode max_bw support_band\n");

	for (i = 0; i < ENV_STAINFO_NUM; i++) {
		if (entry[i].used == _FALSE)
			continue;

		RTW_PRINT_SEL(m, "[%03d] %02x%02x%02x%02x%02x%02x 0x%02x 0x%x 0x%x\n", 
			i, MAC_ARG(entry[i].mac), entry[i].max_mode_support, entry[i].max_bw_support, entry[i].support_band);
	}

	return 0;
}

#endif /* RECORD_ENV_STAINFO */
