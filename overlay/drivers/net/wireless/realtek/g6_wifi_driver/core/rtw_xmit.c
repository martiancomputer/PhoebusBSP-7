...
static s32 update_attrib_sec_info(_adapter *padapter, struct pkt_attrib *pattrib, struct sta_info *psta, enum eap_type eapol_type)
{
	...
	/* Verified on-board: EAPOL always leaves here with blocked=1 encrypt=0, i.e.
	 * unencrypted as it must be while the peer has no PTK, so the encryption
	 * decision is not the problem -- see the hwnat note in network-up. Still
	 * logging it, with psta->state, because reaching this point at all is what
	 * distinguishes "EAPOL was transmitted" from the silent PHOEBUS-TXDROP paths. */
	if (pattrib->ether_type == ETH_P_EAPOL)
		printk(KERN_ERR "PHOEBUS-EAPOL-TX: "MAC_FMT" state=0x%x blocked=%d encrypt=%d macid=%d\n",
			MAC_ARG(pattrib->ra), psta->state, psta->ieee8021x_blocked,
			pattrib->encrypt, psta->phl_sta->macid);
...
get_sta_info:
	...
	if (bmcast) {
		...
		if (psta == NULL) { /* if we cannot get psta => drop the pkt */
			...
			/* Phoebus: this drop is otherwise completely silent (the branch below
			 * prints, this one never did). If EAPOL 1/4 lands here the 4-way can
			 * only time out, which is exactly what we see post-reassociation. */
			printk(KERN_ERR "PHOEBUS-TXDROP(A): "MAC_FMT" ethtype=%04x state=0x%x\n",
				MAC_ARG(XF_RA), XF_ETHTYPE, psta->state);
			return FAIL;
		}
	}
...
get_sta_info:
	...
	if (bmcast) {
		...
		if (psta == NULL) { /* if we cannot get psta => drop the pkt */
			...
			DBG_COUNTER(padapter->tx_logs.core_tx_upd_attrib_err_ucast_ap_link);
			/* Phoebus: see PHOEBUS-TXDROP(A) -- same silent drop, other TX path. */
			printk(KERN_ERR "PHOEBUS-TXDROP(B): "MAC_FMT" ethtype=%04x state=0x%x\n",
				MAC_ARG(pattrib->ra), pattrib->ether_type, psta->state);
			res = _FAIL;
			goto exit;
		}
	}
