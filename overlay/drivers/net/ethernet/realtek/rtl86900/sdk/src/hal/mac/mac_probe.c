...
_hal_portInfo_prepare(rt_device_t *pDev)
{
    ...
    if(RTL9607C_CHIP_ID == pDev->chip_id)
    {
        ...
        {
            /* This register is what actually decides which ports exist, and it
             * silently overrides the static table above. On this board it fuses
             * off port 4, which made the WAN jack look like a driver bug for a
             * long time. Print it: bits 0-4 disable a port, bits 5-9 demote it
             * to 100M. */
            printk("PHOEBUS-CAP: rtl9607c capability = 0x%08x\n", capability);

            for(i = 0 ; i < 5 ; i ++)
            {
                if(capability & (1<<i))
                {
                    printk("PHOEBUS-CAP:   port %u disabled by capability\n", i);
                    pDev->pPortinfo->portType[i] = RT_PORT_NONE;
                }
                else if(capability & (1<<(i+5)))
                {
                    printk("PHOEBUS-CAP:   port %u limited to 100M\n", i);
