...
#else
    /* The GPON lookup is only available when GPON is built in. This file is
     * compiled unconditionally, so with CONFIG_GPON_FEATURE off the reference
     * to rt_gpon_usFlow_get() fails at link:
     *   rt_ponmisc.o: undefined reference to `rt_gpon_usFlow_get'
     * The existing failure path below already describes exactly the right
     * answer for "no GPON" -- flow disabled, no queue, ret OK -- so gate the
     * call and fall into it rather than inventing a second one. */
